...
function parseRepositoryId(value: string): RepoCoordinates {
    ...
    if (!REPOSITORY_ID_PATTERN.test(id)) {
        throw new Error(`Repository id must use the owner/name format: ${value}`);
    }
...
function repositoryIdFromUrl(repositoryUrl: string): string {
    ...
    if (segments.length !== 2 || !segments[0] || !segments[1]) {
        throw new Error("Git URL must map to a two-level owner/name id");
    }
...
function normalizeRepositoryUrl(repositoryUrl: string): string {
    ...
    if (url.username || url.password || url.search || url.hash) {
        throw new Error("Git URLs must not include credentials, query parameters, or fragments");
    }
    return url.toString();
}
...
async function addRepository(idInput: string | undefined, urlInput: string | undefined) {
...
function createMcpServer(): McpServer {
    ...
    server.registerTool(
        "repo_add",
        {
            description: "Add a repository with an HTTPS shallow clone. If id is omitted, derive it from the Git URL owner/name. Run codegraph init after adding it.",
            inputSchema: {
                id: z.string().regex(REPOSITORY_ID_PATTERN).optional().describe("Repository id, for example colbymchenry/codegraph"),
                url: z.string().url().optional().describe("HTTPS Git URL, for example https://github.com/colbymchenry/codegraph"),
            },
        },
        async args => safely(() => addRepository(args.id, args.url)),
    );

    server.registerTool(
        "repo_list",
        {
            description: "List all cloned owner/name repositories and their CodeGraph status.",
            inputSchema: {},
        },
    ...
    server.registerTool(
        "repo_delete",
        {
            description: "Delete a repository and its CodeGraph index.",
            inputSchema: {
                id: z.string().regex(REPOSITORY_ID_PATTERN).describe("Repository id, for example colbymchenry/codegraph"),
            },
        },
    ...
    server.registerTool(
        "codegraph_status",
        {
            description: "Return the CodeGraph index status. Run codegraph init automatically when the repository is not indexed.",
            inputSchema: {
                id: z.string().regex(REPOSITORY_ID_PATTERN).describe("Repository id"),
            },
        },
    ...
    server.registerTool(
        "codegraph_explore",
        {
            description: "Analyze the structure of a repository with CodeGraph. Run codegraph init automatically when the repository is not indexed.",
            inputSchema: {
                id: z.string().regex(REPOSITORY_ID_PATTERN).describe("Repository id"),
                query: z.string().min(1).describe("Natural-language question, file name, or symbol name"),
                maxFiles: z.number().int().min(1).max(20).optional().describe("Maximum number of files to return, default 6"),
            },
        },
